...
#import sys
#sys.path.insert(0, '/Users/irene/Documents/Projects/notes/scripts/sampling/')
from base_Sampler import BaseSampler

def increase_sampling_size(n_explore, buffer, max_queries, n_classes, logger):
    """ When the region corrresponding to a certain class is very small, it takes a lot of time to
...
class ExpandAndContract(BaseSampler):
    ...
    def _explore(self):
        print ("ExpandAndContract ", "_explore")
        ...
        #print ('self.classes_[self.buffer < int(self.max_queries/self.n_classes_)]',
        #       self.classes_[self.buffer < int(self.max_queries/self.n_classes_)])
        #print ('masker',masker)
        #print ('masker shape',masker.shape)
    ...
    def _query(self):
            ...
            if all(self.buffer > 0.) or runs < 2:
                ...
                if self.n_seeds_ > 0:
                    if any(self.buffer[self.sub_sampled] < int(self.max_queries/self.n_classes_)):
                        ...
                        self._exploit_classes(self.classes_[self.sub_sampled][self.buffer[self.sub_sampled] < int(self.max_queries/self.n_classes_)], self.X_, self.y_)
        ...
        if isinstance(self.oracle, BaseEstimator):
            ...
            self.y_ = np.argmax(self.oracle.predict(samples).reshape(-1,1), axis=1).astype(int)
            self.n_queries += len(samples)
            self.buffer = self._compute_buffer()
